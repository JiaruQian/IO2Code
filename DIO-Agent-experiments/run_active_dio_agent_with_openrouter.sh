...
usage() {
  cat <<'EOF'
Usage:
  ./run_active_dio_agent_with_openrouter.sh [--provider openrouter|dashscope|anthropic] single --task <TaskName> [run_active_dio_agent.py args...]
  ./run_active_dio_agent_with_openrouter.sh [--provider openrouter|dashscope|anthropic] all [--start N] [--end M] [run_active_dio_agent.py args shared by every task]
...
Examples:
  ./run_active_dio_agent_with_openrouter.sh --provider openrouter single \
    ...
    --output-subdir active_dio_agent_oracle_abs_current

  ./run_active_dio_agent_with_openrouter.sh --provider dashscope single \
    --task Extra_IsPalindrome \
    --max-iterations 50 \
    --primary-model qwen3.6-plus \
    --output-subdir active_dio_agent_oracle_palindrome

  ./run_active_dio_agent_with_openrouter.sh --provider openrouter all \
    --start 113 --end 115 \
    --max-iterations 50 \
    --primary-model deepseek/deepseek-v3.2 \
    --output-subdir active_dio_agent_oracle_all115
...
if ! has_flag "--primary-model" "${EXTRA_ARGS[@]}"; then
  EXTRA_ARGS+=(--primary-model "${ACTIVE_DIO_AGENT_PRIMARY_MODEL:-${DEFAULT_PRIMARY_MODEL}}")
fi

if [[ "${MODE}" == "single" ]]; then
  python run_active_dio_agent.py "${EXTRA_ARGS[@]}"
  exit $?
fi
...
OUTPUT_SUBDIR="active_dio_agent_oracle_0512"
...
RESULT_DIR="active_dio_agent_batch_results/$(date +%Y%m%d_%H%M%S)"
mkdir -p "${RESULT_DIR}"
...
echo "Running active DIO-Agent on ${TOTAL} tasks (task range ${TASK_START}..${TASK_END}, clamped to first 115 benchmark tasks)."
...
while IFS= read -r task; do
  ...
  start_ts="$(date +%s)"
  set +e
  python run_active_dio_agent.py --task "${task}" "${EXTRA_ARGS[@]}"
  status=$?
  set -e
  end_ts="$(date +%s)"
...
summary_path = Path("tasks") / task / output_subdir / "active_dio_agent_summary.json"
