...
from dio_agent.evolution_trace import EvolutionTracer
from dio_agent.llm.ensemble import LLMEnsemble
from dio_agent.process_parallel import ProcessParallelController
from dio_agent.prompt.sampler import PromptSampler
from dio_agent.utils.code_utils import extract_code_language
from dio_agent.utils.format_utils import format_improvement_safe, format_metrics_safe
...
class DIOAgent:
    ...
    def __init__(
        ...
        self.output_dir = output_dir or os.path.join(
            os.path.dirname(initial_program_path), "dio_agent_output"
        )
        ...
        # Manual mode queue lives in <dio_agent_output>/manual_tasks_queue
        ...
        logger.info(f"Initialized DIOAgent with {initial_program_path}")
    ...
    def _setup_logging(self) -> None:
        ...
        # Add file handler
        log_file = os.path.join(log_dir, f"dio_agent_{time.strftime('%Y%m%d_%H%M%S')}.log")
    ...
    def _setup_manual_mode_queue(self) -> None:
        ...
        Queue directory is always:
          <dio_agent_output>/manual_tasks_queue
    ...
    async def run(
        ...
        if should_add_initial:
            ...
            print(f"\n{'='*70}")
            print(f"🏁 INITIALIZING EVOLUTION")
            print(f"{'='*70}")
            logger.info(f"Adding initial program to all {num_islands} islands for parallel evolution")

            # Evaluate the initial program once (same code, no need to evaluate multiple times)
            print(f"🧪 Evaluating initial program...")
            temp_id = str(uuid.uuid4())
            initial_metrics = await self.evaluator.evaluate_program(
                self.initial_program_code, temp_id
            )
            print(f"📊 Initial program metrics:")
            print(f"   Accuracy: {initial_metrics.get('accuracy', 0.0):.2%}")
            print(f"   Combined Score: {initial_metrics.get('combined_score', 0.0):.4f}")

            # Add initial program copy to each island
            print(f"\n🏝️  Creating {num_islands} island populations...")
            for island_id in range(num_islands):
                ...
                # Add to specific island
                self.database.add(initial_program, target_island=island_id)
                print(f"  ✓ Island {island_id}: {initial_program_id[:8]}... (gen=0)")
                logger.info(f"  ✓ Added initial program to island {island_id} (ID: {initial_program_id[:8]}...)")
            
            print(f"\n🚀 Starting evolution with {num_islands} islands...")
            print(f"{'='*70}\n")
        ...
        try:
            ...
            if self.parallel_controller:
                self._token_usage_summary = self.parallel_controller.get_token_usage_totals()
    ...
    def _save_checkpoint(self, iteration: int) -> None:
        ...
        if best_program:
            token_usage = self._get_token_usage_summary()
    ...
    def _get_token_usage_summary(self) -> Dict[str, int]:
        if self.parallel_controller:
            return self.parallel_controller.get_token_usage_totals()
        return {
            "llm_calls": int(self._token_usage_summary.get("llm_calls", 0)),
            "prompt_tokens": int(self._token_usage_summary.get("prompt_tokens", 0)),
            "completion_tokens": int(self._token_usage_summary.get("completion_tokens", 0)),
            "total_tokens": int(self._token_usage_summary.get("total_tokens", 0)),
        }
