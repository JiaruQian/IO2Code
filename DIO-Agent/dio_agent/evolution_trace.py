"""
Evolution trace logging for DIOAgent
...
from dio_agent.utils.trace_export_utils import (
    append_trace_jsonl,
    export_traces,
    export_traces_json,
)
...
class EvolutionTracer:
    ...
    def __init__(
        ...
        self.iteration_records_dir = self.output_path.parent / "iteration_records"
        self.iteration_records_dir.mkdir(parents=True, exist_ok=True)
    ...
    def log_trace(
        ...
        try:
            ...
            # Persist per-iteration records for easy inspection.
            self._write_iteration_record(trace, parent_program, child_program)
    ...
    def _write_iteration_record(
        self, trace: EvolutionTrace, parent_program: Any, child_program: Any
    ) -> None:
        """
        Persist each iteration into a dedicated folder with source code + metrics.
        ...
        """
        try:
            if isinstance(trace.iteration, int):
                iteration_name = f"iteration_{trace.iteration:06d}"
            else:
                iteration_name = f"iteration_{str(trace.iteration)}"

            iteration_dir = self.iteration_records_dir / iteration_name
            iteration_dir.mkdir(parents=True, exist_ok=True)

            parent_code = getattr(parent_program, "code", "")
            child_code = getattr(child_program, "code", "")

            # Keep language extension aligned with existing output conventions.
            file_ext = ".py"
            parent_language = getattr(parent_program, "language", None)
            child_language = getattr(child_program, "language", None)
            language = child_language or parent_language
            ...
            parent_program_path = iteration_dir / f"parent_program{file_ext}"
            child_program_path = iteration_dir / f"child_program{file_ext}"
            record_path = iteration_dir / "record.json"

            parent_program_path.write_text(parent_code or "", encoding="utf-8")
            child_program_path.write_text(child_code or "", encoding="utf-8")

            record_payload = trace.to_dict()
            record_payload["files"] = {
                "parent_program": str(parent_program_path.name),
                "child_program": str(child_program_path.name),
            }
            with open(record_path, "w", encoding="utf-8") as f:
                json.dump(record_payload, f, ensure_ascii=False, indent=2)
        except Exception as e:
            logger.warning(f"Failed to write iteration record: {e}")
