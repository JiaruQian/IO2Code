...
from dio_agent.config import PromptConfig
from dio_agent.prompt.templates import TemplateManager
from dio_agent.utils.format_utils import format_metrics_safe
...
class PromptSampler:
    ...
    def _format_evolution_history(
        ...
        max_previous_attempts = max(0, self.config.max_previous_attempts)
        if max_previous_attempts > 0:
            selected_previous = previous_programs[
                -min(max_previous_attempts, len(previous_programs)) :
            ]
        else:
            selected_previous = []
    ...
    def _render_artifacts(self, artifacts: Dict[str, Any]) -> str:
        ...
        # Surface failed test samples in a dedicated section so the model can
        # directly use concrete mistakes in the next mutation round.
        curriculum_errors = artifacts.get("curriculum_errors", [])
        replay_errors = artifacts.get("replay_errors", [])
        error_feedback_chain = artifacts.get("error_feedback_chain", [])
        if (
            isinstance(curriculum_errors, list)
            or isinstance(replay_errors, list)
            or isinstance(error_feedback_chain, list)
        ):
            feedback_lines = []

            def _format_error_item(prefix: str, error_item: Any) -> str:
                if not isinstance(error_item, dict):
                    return f"- {prefix}: {repr(error_item)}"
                idx = error_item.get("test_case", "?")
                input_value = error_item.get("input", "<unknown>")
                expected_value = error_item.get("expected")
                actual_value = error_item.get("actual")
                runtime_error = error_item.get("error")
                if runtime_error is not None:
                    return f"- {prefix}#{idx}: input={input_value} | error={runtime_error}"
                return (
                    f"- {prefix}#{idx}: input={input_value} | "
                    f"expected={expected_value} | actual={actual_value}"
                )

            def _append_feedback_block(
                title: str,
                curr_errors: Any,
                rep_errors: Any,
                curr_total: Any,
                ...
                rep_total: Any,
                rep_correct: Any,
                source_program_id: Any = None,
            ) -> None:
                local_lines = [f"### {title}"]
                if isinstance(source_program_id, str) and source_program_id:
                    local_lines.append(f"- Program ID: {source_program_id}")
                if isinstance(curr_correct, int) and isinstance(curr_total, int):
                    local_lines.append(f"- Curriculum: {curr_correct}/{curr_total} passed")
                if isinstance(rep_correct, int) and isinstance(rep_total, int):
                    local_lines.append(f"- Replay: {rep_correct}/{rep_total} passed")

                has_details = False
                if isinstance(curr_errors, list) and curr_errors:
                    has_details = True
                    local_lines.append("")
                    local_lines.append("Failed curriculum cases:")
                    local_lines.extend(_format_error_item("case", item) for item in curr_errors[:3])

                if isinstance(rep_errors, list) and rep_errors:
                    has_details = True
                    local_lines.append("")
                    local_lines.append("Failed replay cases:")
                    local_lines.extend(_format_error_item("replay", item) for item in rep_errors[:3])

                # Only keep blocks with concrete failures.
                if has_details:
                    if feedback_lines:
                        feedback_lines.append("")
                    feedback_lines.extend(local_lines)

            _append_feedback_block(
                title="Parent Program Feedback",
                curr_errors=curriculum_errors,
                rep_errors=replay_errors,
                curr_total=artifacts.get("curriculum_total"),
                curr_correct=artifacts.get("curriculum_correct"),
                rep_total=artifacts.get("replay_total"),
                rep_correct=artifacts.get("replay_correct"),
            )

            if isinstance(error_feedback_chain, list):
                for idx, entry in enumerate(error_feedback_chain, start=1):
                    if not isinstance(entry, dict):
                        continue
                    if idx == 1:
                        title = "Parent's Parent Feedback"
                    else:
                        title = f"Ancestor Feedback (distance {idx + 1})"
                    _append_feedback_block(
                        title=title,
                        curr_errors=entry.get("curriculum_errors", []),
                        rep_errors=entry.get("replay_errors", []),
                        curr_total=entry.get("curriculum_total"),
                        curr_correct=entry.get("curriculum_correct"),
                        rep_total=entry.get("replay_total"),
                        rep_correct=entry.get("replay_correct"),
                        source_program_id=entry.get("program_id"),
                    )

            if feedback_lines:
                sections.append(
                    "## Failure Feedback Chain\n"
                    "Only include this when the selected parent has feedback. "
                    "Use the chain to avoid repeating previously observed failures.\n\n"
                    + "\n".join(feedback_lines)
                )
        ...
        excluded_keys = {
            ...
            "replay_total",
            "replay_correct",
            "error_feedback_chain",
        }

        # Process all other artifacts using .items()
    ...
    def _safe_decode_artifact(self, value: Any) -> str:
        ...
        if isinstance(value, str):
            ...
            try:
                ...
                rendered = str(value)
            if self.config.artifact_security_filter:
                return self._apply_security_filter(rendered)
            return rendered
        else:
            return str(value)
