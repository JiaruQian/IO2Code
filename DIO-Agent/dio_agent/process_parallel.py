...
from dio_agent.database import Program, ProgramDatabase
from dio_agent.utils.metrics_utils import safe_numeric_average
...
def _sanitize_feedback_errors(raw_errors: Any, max_items: int = 3) -> List[Dict[str, Any]]:
    ...
    for item in raw_errors:
        if isinstance(item, dict):
            ...
            sanitized.append({"value": repr(item)})
        if len(sanitized) >= max_items:
            break
    return sanitized


def _extract_error_feedback_snapshot(artifacts: Any) -> Optional[Dict[str, Any]]:
    if not isinstance(artifacts, dict):
        return None

    curriculum_errors = _sanitize_feedback_errors(artifacts.get("curriculum_errors", []))
    replay_errors = _sanitize_feedback_errors(artifacts.get("replay_errors", []))

    # Treat feedback as meaningful only when there are concrete failing cases.
    if not curriculum_errors and not replay_errors:
        return None
    ...
    for key in ("curriculum_total", "curriculum_correct", "replay_total", "replay_correct"):
...
def _build_error_feedback_chain(parent_id: str, parent_artifacts: Any) -> List[Dict[str, Any]]:
    chain: List[Dict[str, Any]] = []
    max_depth = 8

    parent_snapshot = _extract_error_feedback_snapshot(parent_artifacts)
    if parent_snapshot:
        parent_snapshot["program_id"] = parent_id
        parent_snapshot["distance"] = 1
        chain.append(parent_snapshot)

    if isinstance(parent_artifacts, dict):
        inherited_chain = parent_artifacts.get("error_feedback_chain", [])
        if isinstance(inherited_chain, list):
            for entry in inherited_chain:
                if not isinstance(entry, dict):
                    continue
                normalized: Dict[str, Any] = {
                    "curriculum_errors": _sanitize_feedback_errors(entry.get("curriculum_errors", [])),
                    "replay_errors": _sanitize_feedback_errors(entry.get("replay_errors", [])),
                }
                for key in ("program_id", "curriculum_total", "curriculum_correct", "replay_total", "replay_correct"):
                    value = entry.get(key)
                    if isinstance(value, (str, int)):
                        normalized[key] = value

                if not normalized["curriculum_errors"] and not normalized["replay_errors"]:
                    continue
                normalized["distance"] = len(chain) + 1
...
def _lazy_init_worker_components():
    ...
    if _worker_prompt_sampler is None:
        from dio_agent.prompt.sampler import PromptSampler

        _worker_prompt_sampler = PromptSampler(_worker_config.prompt)

    if _worker_evaluator is None:
        from dio_agent.evaluator import Evaluator
        from dio_agent.llm.ensemble import LLMEnsemble
        from dio_agent.prompt.sampler import PromptSampler
...
def _run_iteration_worker(
    ...
    try:
        ...
        print(f"\n{'='*70}")
        print(f"🔄 Iteration {iteration} | Island {parent_island}")
        print(f"{'='*70}")
        print(f"📋 Parent: {parent_id[:8]}... (gen={parent.generation}, score={parent.metrics.get('combined_score', 0.0):.2f})")
        print(f"🏝️  Island {parent_island} has {len(db_snapshot['islands'][parent_island])} programs")
        island_programs = [
        ...
        print(f"🤖 Calling LLM to generate improved code...")
        try:
            ...
            llm_usage = _worker_llm_ensemble.consume_last_usage()
            print(f"✅ LLM response received ({len(llm_response) if llm_response else 0} chars)")
        except Exception as e:
            error_msg = str(e).strip() or repr(e)
            error_type = type(e).__name__
            detailed_error = f"{error_type}: {error_msg}"
            logger.error(f"LLM generation failed: {detailed_error}")
            print(f"❌ LLM generation failed: {detailed_error}")
            return SerializableResult(
                error=f"LLM generation failed: {detailed_error}",
                iteration=iteration,
                llm_usage=llm_usage,
            )
        ...
        if llm_response is None:
            print(f"❌ LLM returned None response")
            return SerializableResult(
        ...
        if _worker_config.diff_based_evolution:
            ...
            print(f"📝 Extracted {len(diff_blocks)} diff blocks from LLM response")
            if not diff_blocks:
                print(f"❌ No valid diffs found in response")
                return SerializableResult(
            ...
            if _worker_config.prompt.programs_as_changes_description:
                try:
                    ...
                    return SerializableResult(error=str(e), iteration=iteration, llm_usage=llm_usage)
            ...
            from dio_agent.utils.code_utils import parse_full_rewrite

            new_code = parse_full_rewrite(llm_response, _worker_config.language)
            if not new_code:
                print(f"❌ No valid code found in response")
                return SerializableResult(
        ...
        # Check code length
        print(f"📏 Generated code: {len(child_code)} chars ({len(child_code.splitlines())} lines)")
        if len(child_code) > _worker_config.max_code_length:
            print(f"❌ Code too long: {len(child_code)} > {_worker_config.max_code_length}")
            return SerializableResult(
        ...
        print(f"💻 Code preview (first 50 lines):")
        code_lines = child_code.splitlines()
        ...
        print(f"   Accuracy: {child_metrics.get('accuracy', 0.0):.2%}")
        print(f"   Combined Score: {child_metrics.get('combined_score', 0.0):.2f}")
        if 'correct' in child_metrics and 'total' in child_metrics:
            print(f"   Tests Passed: {child_metrics['correct']}/{child_metrics['total']}")
        ...
        # Carry parent failure history as a lineage chain so prompt feedback
        # remains aligned under random parent sampling across iterations.
        feedback_chain = _build_error_feedback_chain(parent.id, parent_artifacts)
        if feedback_chain:
            artifacts["error_feedback_chain"] = feedback_chain

        if artifacts:
            curr_errors = artifacts.get("curriculum_errors", [])
            replay_errors = artifacts.get("replay_errors", [])
            if curr_errors:
                print("   Failed curriculum test samples (up to 3):")
                for err in curr_errors[:3]:
                    idx = err.get("test_case")
                    if "actual" in err:
                        print(
                            f"     - case#{idx}: input={err.get('input')} | "
                            f"expected={err.get('expected')} | actual={err.get('actual')}"
                        )
                    else:
                        print(f"     - case#{idx}: input={err.get('input')} | error={err.get('error')}")
            if replay_errors:
                print("   Failed replay test samples (up to 3):")
                for err in replay_errors[:3]:
                    idx = err.get("test_case")
                    if "actual" in err:
                        print(
                            f"     - replay#{idx}: input={err.get('input')} | "
                            f"expected={err.get('expected')} | actual={err.get('actual')}"
                        )
                    else:
                        print(f"     - replay#{idx}: input={err.get('input')} | error={err.get('error')}")
...
class ProcessParallelController:
    ...
    async def run_evolution(
        ...
        while (
            ...
            try:
                ...
                self._accumulate_token_usage(result.llm_usage)

                if result.error:
                    print(f"\n⚠️  Iteration {completed_iteration} failed: {result.error}")
                    ...
                    print(f"\n✅ Program added to database: {child_program.id[:8]}... (gen={child_program.generation})")
                    ...
                    if self.database.best_program_id == child_program.id:
                        print(f"\n🌟 NEW BEST SOLUTION! Iteration {completed_iteration}")
                        print(f"   Score: {child_program.metrics.get('combined_score', 0.0):.4f}")
                        print(f"   Accuracy: {child_program.metrics.get('accuracy', 0.0):.2%}")
                        logger.info(
    ...
    def _accumulate_token_usage(self, usage: Optional[Dict[str, int]]) -> None:
        ...
        if llm_calls <= 0 and (prompt_tokens > 0 or completion_tokens > 0 or total_tokens > 0):
            llm_calls = 1

        self.token_usage_totals["llm_calls"] += llm_calls
        self.token_usage_totals["prompt_tokens"] += prompt_tokens
        self.token_usage_totals["completion_tokens"] += completion_tokens
        self.token_usage_totals["total_tokens"] += total_tokens

    def get_token_usage_totals(self) -> Dict[str, int]:
        return {
            "llm_calls": int(self.token_usage_totals.get("llm_calls", 0)),
            "prompt_tokens": int(self.token_usage_totals.get("prompt_tokens", 0)),
            "completion_tokens": int(self.token_usage_totals.get("completion_tokens", 0)),
            "total_tokens": int(self.token_usage_totals.get("total_tokens", 0)),
        }
    ...
    def _submit_iteration(
        ...
        try:
            ...
            num_inspirations = self.config.prompt.num_inspirations
            ...
            parent, inspirations = self.database.sample_from_island(
                island_id=target_island, num_inspirations=num_inspirations
            )
